...
const API_BASE = "http://182.180.69.171:5000/api/billing";
...
const fetchDeviceNames = async () => {
  try {
    setLoading(true);
    const response = await fetch(`${API_BASE}/device-names`);
...
// 2. Fetch meters for a selected device
const fetchMetersForDevice = async (deviceName) => {
  try {
    setLoading(true);
    const response = await fetch(`${API_BASE}/devices/${deviceName}/meters`);
...
// 3. Fetch meter data for selected meters and date range
const fetchMeterData = async () => {
  if (selectedMeters.length === 0) return;
  try {
    setLoading(true);
    const response = await fetch(`${API_BASE}/meters/data?startDate=${dateRange.startDate}&endDate=${dateRange.endDate}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ meterIds: selectedMeters.map(m => m.dev_eui) })
    });
...
const calculateAggregatedData = (data) => {
  // Sort by received_at to ensure correct first/last
  ...
  data.forEach(record => {
    const importVal = record.energy_import ?? null;
    const exportVal = record.energy_export ?? null;
    const voltageVal = record.voltage ?? null;
    const currentVal = record.current ?? null;
    const demandVal = record.max_demand ?? 0;

    if (voltageVal !== null) voltageSum += voltageVal;
    if (currentVal !== null) currentSum += currentVal;
    if (voltageVal !== null || currentVal !== null) count++;

    if (demandVal > aggregated.maxDemand) aggregated.maxDemand = demandVal;

    // ---------- Daily Usage ----------
    const day = record.received_at.slice(0, 10); // YYYY-MM-DD
    if (!aggregated.dailyUsage[day]) {
      aggregated.dailyUsage[day] = {
        date: day,
        firstImport: importVal,
        lastImport: importVal,
        firstExport: exportVal,
        lastExport: exportVal,
        demand: demandVal
      };
    } else {
      if (importVal !== null) aggregated.dailyUsage[day].lastImport = importVal;
      if (exportVal !== null) aggregated.dailyUsage[day].lastExport = exportVal;
      aggregated.dailyUsage[day].demand = Math.max(aggregated.dailyUsage[day].demand, demandVal);
    }
    ...
    if (!aggregated.meterSummary[record.dev_eui]) {
      ...
      if (importVal !== null) aggregated.meterSummary[record.dev_eui].lastImport = importVal;
      if (exportVal !== null) aggregated.meterSummary[record.dev_eui].lastExport = exportVal;
    ...
    if (record.received_at) {
      ...
      if (!aggregated.last6Months[monthKey]) {
        aggregated.last6Months[monthKey] = {
          month: new Date(record.received_at).toLocaleDateString('en-US', {
            month: 'long',
            year: 'numeric'
          }),
          firstImport: importVal,
        ...
        if (importVal !== null) aggregated.last6Months[monthKey].lastImport = importVal;
        aggregated.last6Months[monthKey].demand = Math.max(
          aggregated.last6Months[monthKey].demand,
          demandVal
        );
      }
    }
  ...
  // Finalize averages
  aggregated.avgVoltage = count > 0 ? voltageSum / count : 0;
  aggregated.avgCurrent = count > 0 ? currentSum / count : 0;

  // ---------- Finalize daily usage ----------
  Object.keys(aggregated.dailyUsage).forEach(day => {
    const d = aggregated.dailyUsage[day];
    d.consumption = (d.firstImport !== null && d.lastImport !== null)
      ? Math.max(0, d.lastImport - d.firstImport)
      : 0;
    d.export = (d.firstExport !== null && d.lastExport !== null)
      ? Math.max(0, d.lastExport - d.firstExport)
      : 0;
  });

  // ---------- Finalize meter summary ----------
  Object.keys(aggregated.meterSummary).forEach(meterId => {
    const m = aggregated.meterSummary[meterId];
    m.consumption = (m.firstImport !== null && m.lastImport !== null)
      ? Math.max(0, m.lastImport - m.firstImport)
      : 0;
    m.export = (m.firstExport !== null && m.lastExport !== null)
      ? Math.max(0, m.lastExport - m.firstExport)
      : 0;
  });

  // ---------- Totals ----------
  aggregated.totalConsumption = Object.values(aggregated.meterSummary)
    .reduce((sum, m) => sum + (m.consumption || 0), 0);
  aggregated.totalExport = Object.values(aggregated.meterSummary)
    .reduce((sum, m) => sum + (m.export || 0), 0);

  // ---------- Finalize last 6 months ----------
  Object.keys(aggregated.last6Months).forEach(month => {
    const d = aggregated.last6Months[month];
    d.consumption = (d.firstImport !== null && d.lastImport !== null)
      ? Math.max(0, d.lastImport - d.firstImport)
      : 0;
    d.cost = calculateElectricityCost(d.consumption, d.demand);
  });
  ...
  setAggregatedData(aggregated);
    ...
    useEffect(() => { fetchDeviceNames(); }, []);
  useEffect(() => { if (selectedDeviceName) fetchMetersForDevice(selectedDeviceName); }, [selectedDeviceName]);
  useEffect(() => { if (selectedMeters.length > 0) fetchMeterData(); }, [selectedMeters, dateRange]);
